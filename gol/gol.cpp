...
bool InitOpenCL (void)
{
    ...
    if (context == nullptr || !CheckCLError (err)) return false;
    ...
    if (commands == nullptr || !CheckCLError (err)) return false;
    ...
    if (deviceBuffer == nullptr || !CheckCLError (err)) return false;

    deviceBuffer_out = clCreateBuffer (context, CL_MEM_READ_WRITE, NUM_OF_CELLS, nullptr, &err);
    if (deviceBuffer_out == nullptr || !CheckCLError (err)) return false;
